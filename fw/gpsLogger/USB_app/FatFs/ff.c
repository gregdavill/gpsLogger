...
#define	FS_DIR_Name			0		/* Short file name (11) */
#define	FS_DIR_Attr			11		/* Attribute (1) */
#define	FS_DIR_NTres			12		/* Lower case flag (1) */
#define FS_DIR_CrtTimeTenth	13		/* Created time sub-second (1) */
#define	FS_DIR_CrtTime			14		/* Created time (2) */
#define	FS_DIR_CrtDate			16		/* Created date (2) */
#define FS_DIR_LstAccDate		18		/* Last accessed date (2) */
#define	FS_DIR_FstClusHI		20		/* Higher 16-bit of first cluster (2) */
#define	FS_DIR_WrtTime			22		/* Modified time (2) */
#define	FS_DIR_WrtDate			24		/* Modified date (2) */
#define	FS_DIR_FstClusLO		26		/* Lower 16-bit of first cluster (2) */
#define	FS_DIR_FileSize		28		/* File size (4) */
#define	LFS_DIR_Ord			0		/* LFN entry order and LLE flag (1) */
#define	LFS_DIR_Attr			11		/* LFN attribute (1) */
#define	LFS_DIR_Type			12		/* LFN type (1) */
#define	LFS_DIR_Chksum			13		/* Checksum of corresponding SFN entry */
#define	LFS_DIR_FstClusLO		26		/* Must be zero (0) */
#define	SZ_FS_DIRE				32		/* Size of a directory entry */
#define	LLEF				0x40	/* Last long entry flag in LFS_DIR_Ord */
#define	DDEM				0xE5	/* Deleted directory entry mark at FS_DIR_Name[0] */
...
FRESULT chk_lock (	/* Check if the file can be accessed */
	FS_DIR* dp,		/* Directory object pointing the file to be checked */
...
UINT inc_lock (	/* Increment object open counter and returns its index (0:Internal error) */
	FS_DIR* dp,	/* Directory object pointing the file to register or increment */
...
FRESULT dir_sdi (	/* FR_OK(0):succeeded, !=0:error */
	FS_DIR* dp,		/* Pointer to directory object */
...
)
{
	...
	if (clst == 0) {	/* Static table (root-directory in FAT12/16) */
		...
		ic = SS(dp->fs) / SZ_FS_DIRE * dp->fs->csize;	/* Entries per cluster */
	...
	dp->sect = sect + idx / (SS(dp->fs) / SZ_FS_DIRE);					/* Sector# of the directory entry */
	dp->dir = dp->fs->win + (idx % (SS(dp->fs) / SZ_FS_DIRE)) * SZ_FS_DIRE;	/* Ptr to the entry in the sector */
...
FRESULT dir_next (	/* FR_OK(0):succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
	FS_DIR* dp,		/* Pointer to the directory object */
...
)
{
	...
	if (!(i % (SS(dp->fs) / SZ_FS_DIRE))) {	/* Sector changed? */
		dp->sect++;					/* Next sector */
		...
		if (!dp->clust) {		/* Static table */
			...
			if (((i / (SS(dp->fs) / SZ_FS_DIRE)) & (dp->fs->csize - 1)) == 0) {	/* Cluster changed? */
	...
	dp->dir = dp->fs->win + (i % (SS(dp->fs) / SZ_FS_DIRE)) * SZ_FS_DIRE;	/* Current entry in the window */
...
)
{
	...
	cl = LD_WORD(dir + FS_DIR_FstClusLO);
	if (fs->fs_type == FS_FAT32)
		cl |= (DWORD)LD_WORD(dir + FS_DIR_FstClusHI) << 16;
...
{
	ST_WORD(dir + FS_DIR_FstClusLO, cl);
	ST_WORD(dir + FS_DIR_FstClusHI, cl >> 16);
}
#endif
...
)
{
	...
	if (LD_WORD(dir + LFS_DIR_FstClusLO) != 0) return 0;	/* Check LFS_DIR_FstClusLO */

	i = ((dir[LFS_DIR_Ord] & 0x3F) - 1) * 13;	/* Offset in the LFN buffer */
	...
	if ((dir[LFS_DIR_Ord] & LLEF) && wc && lfnbuf[i])	/* Last segment matched but different length */
		return 0;
...
)
{
	...
	if (dir[LFS_DIR_Ord] & LLEF) {				/* Put terminator if it is the last LFN part */
		if (i >= _MAX_LFN) return 0;		/* Buffer overflow? */
		lfnbuf[i] = 0;
	}
...
)
{
	...
	dir[LFS_DIR_Chksum] = sum;			/* Set checksum */
	dir[LFS_DIR_Attr] = AM_LFN;		/* Set attribute. LFN entry */
	dir[LFS_DIR_Type] = 0;
	ST_WORD(dir + LFS_DIR_FstClusLO, 0);
	...
	dir[LFS_DIR_Ord] = ord;			/* Set the LFN order */
}
...
FRESULT dir_find (	/* FR_OK(0):succeeded, !=0:error */
	FS_DIR* dp			/* Pointer to the directory object linked to the file name */
)
{
	FRESULT res;
	BYTE c, *dir;
	...
	do {
			...
			if (a == AM_LFN) {			/* An LFN entry is found */
				if (dp->lfn) {
					if (c & LLEF) {		/* Is it start of LFN sequence? */
						sum = dir[LFS_DIR_Chksum];
						c &= ~LLEF; ord = c;	/* LFN start order */
						dp->lfn_idx = dp->index;	/* Start index of LFN */
					}
					/* Check validity of the LFN entry and compare it with given name */
					ord = (c == ord && sum == dir[LFS_DIR_Chksum] && cmp_lfn(dp->lfn, dir)) ? ord - 1 : 0xFF;
				}
		...
		if (!(dir[FS_DIR_Attr] & AM_VOL) && !mem_cmp(dir, dp->fn, 11)) /* Is it a valid entry? */
			break;
#endif
	...
	while (dp->sect) {
		...
		if (c == DDEM || (!_FS_RPATH && c == '.') || (int)((a & ~AM_ARC) == AM_VOL) != vol) {	/* An entry without valid data */
			...
			if (a == AM_LFN) {			/* An LFN entry is found */
				...
				ord = (c == ord && sum == dir[LFS_DIR_Chksum] && pick_lfn(dp->lfn, dir)) ? ord - 1 : 0xFF;
			} else {					/* An SFN entry is found */
...
FRESULT dir_register (	/* FR_OK:succeeded, FR_DENIED:no free entry or too many SFN collision, FR_DISK_ERR:disk error */
	FS_DIR* dp				/* Target directory with object name to be created */
)
{
	FRESULT res;
#if _USE_LFN	/* LFN configuration */
	...
	if (res == FR_OK) {				/* Set SFN entry */
		res = move_window(dp->fs, dp->sect);
		if (res == FR_OK) {
			mem_set(dp->dir, 0, SZ_FS_DIRE);	/* Clean the entry */
			mem_cpy(dp->dir, dp->fn, 11);	/* Put SFN */
#if _USE_LFN
			dp->dir[FS_DIR_NTres] = dp->fn[NSFLAG] & (NS_BODY | NS_EXT);	/* Put NT flag */
#endif
			dp->fs->wflag = 1;
		}
	}
...
FRESULT dir_remove (	/* FR_OK:Succeeded, FR_DISK_ERR:A disk error */
	FS_DIR* dp				/* Directory object pointing the entry to be removed */
)
{
	FRESULT res;
#if _USE_LFN	/* LFN configuration */
	...
	if (res == FR_OK) {
		do {
			res = move_window(dp->fs, dp->sect);
			if (res != FR_OK) break;
			mem_set(dp->dir, 0, SZ_FS_DIRE);	/* Clear and mark the entry "deleted" */
			*dp->dir = DDEM;
			dp->fs->wflag = 1;
...
)
{
	...
	if (dp->sect) {		/* Get SFN */
...
#if _USE_LFN
			if (IsUpper(c) && (dir[FS_DIR_NTres] & (i >= 9 ? NS_EXT : NS_BODY)))
				c += 0x20;			/* To lower */
#if _LFN_UNICODE
		...
		fno->fattrib = dir[FS_DIR_Attr];				/* Attribute */
		fno->fsize = LD_DWORD(dir + FS_DIR_FileSize);	/* Size */
		fno->fdate = LD_WORD(dir + FS_DIR_WrtDate);	/* Date */
		fno->ftime = LD_WORD(dir + FS_DIR_WrtTime);	/* Time */
	}
...
FRESULT follow_path (	/* FR_OK(0): successful, !=0: error code */
	FS_DIR* dp,			/* Directory object to return last directory and found object */
...
)
{
	...
	if ((UINT)*path < ' ') {				/* Null path name is the origin directory itself */
		...
		for (;;) {
			...
			if (!(dir[FS_DIR_Attr] & AM_FS_DIR)) {	/* It is not a sub-directory and cannot follow */
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if (fs->n_rootdir % (SS(fs) / SZ_FS_DIRE))				/* (Must be sector aligned) */
		return FR_NO_FILESYSTEM;
	...
	sysect = nrsv + fasize + fs->n_rootdir / (SS(fs) / SZ_FS_DIRE);	/* RSV + FAT + FS_DIR */
...
FRESULT validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
	void* obj		/* Pointer to the object FIL/FS_DIR to check validity */
)
{
	FIL *fil = (FIL*)obj;	/* Assuming offset of .fs and .id in the FIL/FS_DIR structure is identical */
	...
	if (res == FR_OK) {
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
			if (res != FR_OK) {					/* No file, create new */
				...
				if (dir[FS_DIR_Attr] & (AM_RDO | AM_FS_DIR)) {	/* Cannot overwrite it (R/O or FS_DIR) */
					res = FR_DENIED;
				} else {
			...
			if (res == FR_OK && (mode & FA_CREATE_ALWAYS)) {	/* Truncate it if overwrite mode */
				dw = GET_FATTIME();
				ST_DWORD(dir + FS_DIR_CrtTime, dw);/* Set created time */
				ST_DWORD(dir + FS_DIR_WrtTime, dw);/* Set modified time */
				dir[FS_DIR_Attr] = 0;				/* Reset attribute */
				ST_DWORD(dir + FS_DIR_FileSize, 0);/* Reset file size */
			...
			if (res == FR_OK) {					/* Following succeeded */
				if (dir[FS_DIR_Attr] & AM_FS_DIR) {	/* It is a directory */
					res = FR_NO_FILE;
				} else {
					if ((mode & FA_WRITE) && (dir[FS_DIR_Attr] & AM_RDO)) /* R/O violation */
						res = FR_DENIED;
				}
			}
		}
		...
		if (res == FR_OK) {
			...
			fp->fsize = LD_DWORD(dir + FS_DIR_FileSize);	/* File size */
...
)
{
	...
	if (res == FR_OK) {
		if (fp->flag & FA__WRITTEN) {	/* Is there any change to the file? */
			...
			if (res == FR_OK) {
				dir = fp->dir_ptr;
				dir[FS_DIR_Attr] |= AM_ARC;					/* Set archive bit */
				ST_DWORD(dir + FS_DIR_FileSize, fp->fsize);	/* Update file size */
				st_clust(dir, fp->sclust);					/* Update start cluster */
				tm = GET_FATTIME();							/* Update modified time */
				ST_DWORD(dir + FS_DIR_WrtTime, tm);
				ST_WORD(dir + FS_DIR_LstAccDate, 0);
				fp->flag &= ~FA__WRITTEN;
				fp->fs->wflag = 1;
				res = sync_fs(fp->fs);
			}
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {					/* Follow completed */
			if (!dj.dir) {
				...
				if (dj.dir[FS_DIR_Attr] & AM_FS_DIR)	/* Reached to the directory */
...
FRESULT f_opendir (
	FS_DIR* dp,			/* Pointer to directory object to create */
	const TCHAR* path	/* Pointer to the directory path */
)
{
	FRESULT res;
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Follow completed */
			if (dp->dir) {						/* It is not the origin directory itself */
				if (dp->dir[FS_DIR_Attr] & AM_FS_DIR)	/* The object is a sub directory */
...
FRESULT f_closedir (
	FS_DIR *dp		/* Pointer to the directory object to be closed */
)
{
	FRESULT res;
...
FRESULT f_readdir (
	FS_DIR* dp,			/* Pointer to the open directory object */
...
FRESULT f_findfirst (
	FS_DIR* dp,				/* Pointer to the blank directory object */
	FILINFO* fno,			/* Pointer to the file information structure */
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {					/* The object is accessible */
			...
			if (!dir) {
				...
				if (dir[FS_DIR_Attr] & AM_RDO)
					res = FR_DENIED;		/* Cannot remove R/O object */
			}
			if (res == FR_OK) {
				dclst = ld_clust(dj.fs, dir);
				if (dclst && (dir[FS_DIR_Attr] & AM_FS_DIR)) {	/* Is it a sub-directory ? */
#if _FS_RPATH
...
#endif
					{
						mem_cpy(&sdj, &dj, sizeof (FS_DIR));	/* Open the sub-directory */
						sdj.sclust = dclst;
						res = dir_sdi(&sdj, 2);
						if (res == FR_OK) {
	...
	if (res == FR_OK) {
		...
		if (res == FR_NO_FILE) {				/* Can create a new directory */
			...
			if (res == FR_OK) {					/* Initialize the new directory table */
				...
				mem_set(dir + FS_DIR_Name, ' ', 11);	/* Create "." entry */
				dir[FS_DIR_Name] = '.';
				dir[FS_DIR_Attr] = AM_FS_DIR;
				ST_DWORD(dir + FS_DIR_WrtTime, tm);
				st_clust(dir, dcl);
				mem_cpy(dir + SZ_FS_DIRE, dir, SZ_FS_DIRE); 	/* Create ".." entry */
				dir[SZ_FS_DIRE + 1] = '.'; pcl = dj.sclust;
				if (dj.fs->fs_type == FS_FAT32 && pcl == dj.fs->dirbase)
					pcl = 0;
				st_clust(dir + SZ_FS_DIRE, pcl);
			...
			if (res != FR_OK) {
				...
				dir = dj.dir;
				dir[FS_DIR_Attr] = AM_FS_DIR;				/* Attribute */
				ST_DWORD(dir + FS_DIR_WrtTime, tm);	/* Created time */
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {						/* Is it a root directory? */
				...
				dir[FS_DIR_Attr] = (attr & mask) | (dir[FS_DIR_Attr] & (BYTE)~mask);	/* Apply attribute change */
				dj.fs->wflag = 1;
				res = sync_fs(dj.fs);
			}
		}
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Old object is found */
			if (!djo.dir) {						/* Is root dir? */
				...
				mem_cpy(buf, djo.dir + FS_DIR_Attr, 21);	/* Save information about object except name */
				mem_cpy(&djn, &djo, sizeof (FS_DIR));		/* Duplicate the directory object */
...
/* Start of critical section where any interruption can cause a cross-link */
						...
						dir[FS_DIR_Attr] = buf[0] | AM_ARC;
						djo.fs->wflag = 1;
						if ((dir[FS_DIR_Attr] & AM_FS_DIR) && djo.sclust != djn.sclust) {	/* Update .. entry in the sub-directory if needed */
							dw = clust2sect(djo.fs, ld_clust(djo.fs, dir));
							if (!dw) {
								res = FR_INT_ERR;
							} else {
								res = move_window(djo.fs, dw);
								dir = djo.fs->win + SZ_FS_DIRE * 1;	/* Ptr to .. entry */
								if (res == FR_OK && dir[1] == '.') {
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {					/* Root directory */
				...
				ST_WORD(dir + FS_DIR_WrtTime, fno->ftime);
				ST_WORD(dir + FS_DIR_WrtDate, fno->fdate);
				dj.fs->wflag = 1;
				res = sync_fs(dj.fs);
			}
		}
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {			/* A volume label is found */
			if (vn[0]) {
				...
				tm = GET_FATTIME();
				ST_DWORD(dj.dir + FS_DIR_WrtTime, tm);
			} else {
			...
			if (res == FR_NO_FILE) {
				...
				if (vn[0]) {				/* Create volume label as new */
					...
					if (res == FR_OK) {
						mem_set(dj.dir, 0, SZ_FS_DIRE);	/* Set volume label */
						mem_cpy(dj.dir, vn, 11);
						dj.dir[FS_DIR_Attr] = AM_VOL;
						tm = GET_FATTIME();
						ST_DWORD(dj.dir + FS_DIR_WrtTime, tm);
						dj.fs->wflag = 1;
						res = sync_fs(dj.fs);
...
/*-----------------------------------------------------------------------*/
#define N_ROOTFS_DIR	512		/* Number of root directory entries for FAT12/16 */
...
)
{
	...
	if (fmt == FS_FAT32) {
		...
		n_rsv = 1;
		n_dir = (DWORD)N_ROOTFS_DIR * SZ_FS_DIRE / SS(fs);
	}
