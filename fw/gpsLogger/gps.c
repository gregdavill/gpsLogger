...
void gps_ublox_send() {
	const char change_talker[] = { 0xB5, 0x62, 0x06, 0x17, 0x14, 0x00, 0x00,
			0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01,
			'G', 'P', 0, 0, 0, 0, 0, 0 };
	...
	/* Disable GLL */
	const char disable_gll[] = { 0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x01,
			0x00 };
	ubx_transmit_message(disable_gll, sizeof(disable_gll));

	/* Slow GSV */
	const char disable_gsv[] = { 0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x03,
			0x10 };
	ubx_transmit_message(disable_gsv, sizeof(disable_gsv));

	/* Slow GSA */
	const char disable_gsa[] = { 0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x02,
			0x10 };
	ubx_transmit_message(disable_gsa, sizeof(disable_gsa));

	/* Disable VTG */
	const char disable_vtg[] = { 0xB5, 0x62, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x05,
			0x00 };
	ubx_transmit_message(disable_vtg, sizeof(disable_vtg));

	uint16_t transmission_speed = 500;
	/* Boost Transmit rate to 2Hz */
	const char boost_rate[] = { 0xB5, 0x62, 0x06, 0x08, 0x06, 0x00,
			(char) (transmission_speed & 0xFF),
			(char) (transmission_speed >> 8), 1, 0, 0, 0 };
		...
		while (fifo_available()) {
			if (gps_util_valid(fifo_pop())) /* returns true when a correct checksum is processed */
			{
...

				{
					...
					rc = f_write(&gps_log, (const void*) gps_line, len,
							(unsigned int*) &bw); /* Write data to the file */
					if (rc || len != bw) {
						hal_led_a(YELLOW);
					}
				}
				...
				if (gps_util_is_RMC(gps_line)) {
					...
					if (gps_util_fix_valid(gps_line)) {
						...
						//if (gps_time_invalid) {
							...
							if (gps_util_extract_date(gps_line, file_date)
									!= 0) {
